...
import { createSaleTransaction, getAllProducts } from '../../../Redux/slices/AdminReducer';

function AddSaleTransactionAdmin() {
    const navigate = useNavigate();
    const dispatch = useDispatch()
    const { enqueueSnackbar } = useSnackbar();

    const dataProduct = useSelector(state => state.admin.getDataProducts);
    React.useEffect(() => {
    ...
    const handleCreateSaleTransaction = async (e) => {
        e.preventDefault()
        const sale = {
        ...
        dispatch(createSaleTransaction(sale)).then((res) => {
            if (res.payload.status === true || res.payload.statusCode === 201) {
                ...
                navigate('/admin/sale-transactionn')
            ...
            <Box sx={{ display: 'flex', width: '100%', maxWidth: '1440px', border: '1px solid #101010', borderRadius: '8px', height: '95%', mt: '30px' }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%', maxWidth: '1440px', gap: '25px' }}>
                        ...
                        <Box sx={{ display: 'flex', gap: '36px', width: '100%', maxWidth: '1440px', justifyContent: 'center' }}>
                            ...
                            <Button onClick={handleCreateSaleTransaction} variant='contained' sx={{
...
export default AddSaleTransactionAdmin